...
class Uphill : public GameArea
{
public:

	Uphill(b2World* world, const b2Vec2& pos, const Theme* theme, int variant) : GameArea(world, pos, theme, variant)
	{
		...
		const float endx = (m_variant == 1) ? 50.0f : 80.0f;
		const float starty = 0.0f;
		const float factor = (m_variant == 1) ? 0.5f : 0.4f;
